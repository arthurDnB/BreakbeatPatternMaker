...
Playback, undo/redo and WAV export stay in a sticky transport. The compact generator and A/B/C/Fill bank sit above the tracker. Use the slot ellipsis to rename; clicking an empty slot still copies the current pattern. Click a populated tracker hit to select and audition that hit; click an empty cell to audition its lane sound. Use arrow keys to move the cursor, Shift+arrows or Shift-click to select a rectangular cell range, and Ctrl/Cmd-click to add or remove individual cells, including empty ones. Delete, lock, mutate and scramble act on the selected hits in those cells. The top tempo value is editable: it changes pattern BPM when the transport target is Pattern and song BPM when the target is Song; pattern tempo edits support Undo/Redo.
