...
Tracker view is the default. Each hit shows Note, Instrument, Volume, Pan, Delay, and FX above its sound name. Click a value or use Left/Right or Tab/Shift+Tab to move between values; Up/Down changes rows. Tab wraps to the next lane after FX. Note keys enter chromatic notes; Volume, Pan, and Delay accept two hexadecimal digits (00–80 for Volume/Pan, 00–FF for Delay). The FX field accepts a command letter followed by two hex digits, optionally prefixed with `0`: `S80` offsets a sample halfway, `B00` reverses it, `U10`/`D10` slide pitch, `C06` cuts after six ticks, and `R03` retriggers every three ticks. Older `09`, `01`, and `02` spellings are accepted as offset and pitch-slide aliases. Delete in the FX field clears only that effect. These are local hit effects for this app, inspired by the [Renoise effect command reference](https://tutorials.renoise.com/wiki/Effect_Commands); they do not imply native Renoise project export. Enter on Instrument opens a per-hit sound picker; its Lane option follows the track instrument, while a library or uploaded sample is saved with that hit. Escape cancels an incomplete hexadecimal entry. Click and drag the sound-name area to move a hit or selected cell group; a highlighted destination shows where it will land. Locked sources or destinations reject the whole move, and one Undo restores it. The selected-hit editor below the tracker handles pitch, reverse, ratchets, gate, and less common settings.

Select cells or rows, then use Copy, Paste, or Duplicate (Ctrl/Cmd+C/V/D). Paste includes empty cells and replaces destination cells in one undoable edit; locked lanes and hits block the whole paste. The clipboard works within the app while it is open and requires matching tracker resolution. Space plays/stops, Delete clears selected unlocked hits, and typing into a form field never records notes. Manual insertion can use a lane excluded from generation; exclusion applies to automatic generation, not composition.
...
Genre, BPM, Complexity and Bars stay visible. Advanced generation groups Pattern, Groove, and Structure settings. Starting bar appears only for a longer phrase that offers multiple starting positions. Play/Stop and tempo sit centrally above the generator; Master DSP has its own collapsed row. More contains Copy/Download pattern JSON and closes after use, on outside click, or Escape.
...
Select a tracker hit, open the compact Hit editor, choose 1–8 ratchets and a gate length, then Apply to selected hit. Preview selected hit plays that saved hit through the same engine as pattern/arrangement playback and WAV export. Insert and keyboard entry use the displayed articulation settings too.
...
Playback, undo/redo and WAV export sit in a transport immediately above the generator. The A/B/C/Fill bank sits beside the tracker. Use the slot ellipsis to rename; clicking an empty slot still copies the current pattern. Click a populated tracker hit to select and audition that hit; click an empty cell to audition its lane sound. Shift+arrows or Shift-click select a rectangular cell range, and Ctrl/Cmd-click adds or removes individual cells, including empty ones. Delete, lock, mutate and scramble act on the selected hits in those cells. The transport tempo value is editable: it changes pattern BPM when the transport target is Pattern and song BPM when the target is Song; pattern tempo edits support Undo/Redo.

Each track header has an Instrument / FX accordion with sample choice, upload, lane controls and effects. Sounds opens the current track's accordion. The streamlined hit editor sits below the tracker, and beat explanations appear dynamically in the top-right header badge above the grid. Drag the vertical divider to resize the pattern bank, drag the divider above the transport to resize the tracker, or choose Stack panels to place bank and tracker one above the other. Sizes and stacking are remembered locally. On narrow screens the panels stack automatically. The Waveform Slicer is hidden; older slice references still play and remain in saved projects.
